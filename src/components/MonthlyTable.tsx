...
const MonthDailyBreakdown: React.FC<MonthDailyBreakdownProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-indigo-200 shadow-sm p-4 sm:p-5 space-y-4">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between pb-3 border-b border-slate-200 gap-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="bg-indigo-600 text-white text-xs font-bold px-2.5 py-1 rounded flex items-center gap-1.5 shadow-xs">
              ...
              {m.label} 日別データ推移・1日平均利益 累積積み上げ乖離
            </span>
          ...
          <p className="text-xs text-slate-500 mt-1">
            当月の1日平均利益（<strong className="text-slate-800">{formatYenExact(monthDailyAvg)}/日</strong>）を基準として、日々の営業粗利との乖離（単日上振れ/還元）および日が進むごとの<strong>「積み上げ累積乖離」</strong>を可視化しています。
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 self-start lg:self-center flex-wrap">
          <div className="text-right">
            <div className="text-[11px] text-slate-500">当月1日平均利益</div>
          ...
          <div className="text-right">
            <div className="text-[11px] text-slate-500">月間ホール粗利計</div>
            <div className="text-sm font-black text-slate-900">{formatYenExact(gYen)}</div>
          </div>
          <button
            type="button"
        ...
        <div className="bg-slate-50/70 rounded-xl border border-slate-200/80 p-3.5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-2 mb-2 border-b border-slate-200 text-xs">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="font-bold text-slate-800">
                1日平均比 乖離推移 & 積み上げ累積ペースグラフ
              </span>
              <span className="text-[11px] text-slate-500">
                (棒: 単日乖離 / 折れ線: 日次積み上げ累積乖離)
              </span>
            </div>
            <div className="flex items-center gap-3 text-[11px] mt-1 sm:mt-0 font-medium flex-wrap">
              <span className="inline-flex items-center gap-1">
                ...
                {perspective === 'hall' ? '単日上振れ(黒字回収)' : '単日客負け'}
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="w-2.5 h-2.5 bg-rose-500 rounded-xs" />
                {perspective === 'hall' ? '単日還元(出玉放出)' : '単日客勝ち'}
              </span>
          ...
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={dailyItems} margin={{ top: 12, right: 12, left: 0, bottom: 0 }}>
                ...
                <RechartsTooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-slate-900/95 text-white p-3 rounded-lg shadow-xl text-xs space-y-1.5 border border-slate-700 min-w-[220px]">
                          ...
                          <div className="space-y-1 text-[11px] pt-0.5">
                            <div className="flex justify-between">
                              <span className="text-slate-400">{perspective === 'hall' ? '当日粗利:' : '当日客収支:'}</span>
                              <span className="font-bold text-white">{formatYen(data.dayProfit)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-slate-400">当月1日平均:</span>
                            ...
                            <div className="flex justify-between font-bold pt-0.5 border-t border-slate-800">
                              <span className="text-slate-300">単日乖離 (1日平均比):</span>
                              <span className={isPositive ? 'text-emerald-400' : 'text-rose-400'}>
                                {isPositive ? '+' : ''}{formatYen(data.dayDeviation)}
                              </span>
                            </div>
                            <div className="flex justify-between pt-0.5 border-t border-slate-800 font-extrabold text-xs">
                              <span className="text-sky-300">積み上げ累積乖離:</span>
                              <span className={isCumPositive ? 'text-sky-300' : 'text-rose-300'}>
                                {isCumPositive ? '+' : ''}{formatYen(data.cumDeviation)}
                              </span>
                            </div>
                            <div className="text-[10px] py-0.5 px-1.5 rounded bg-slate-800/80 text-center font-medium text-slate-300">
                              {isCumPositive ? '★ 当月平均先行ペース (店舗黒字優位)' : '◆ 還元先行期 (ユーザー有利ペース)'}
                            </div>
                ...
                />
                <Bar dataKey="dayDeviation" yAxisId="yen" name="単日乖離" opacity={0.7} radius={[3, 3, 0, 0]}>
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-500 pt-2 border-t border-slate-200/70">
            <span className="flex items-center gap-1">
              ...
              <span>
                <strong>月末着地点について:</strong> 月間1日平均との各日の乖離を全日積み上げると、月末最終日に理論上ぴったり<strong>0円</strong>に着地・収束します。
              </span>
            </span>
            {firstTurnPositiveDay && (
              <span className="text-indigo-700 font-bold bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200 hidden md:inline">
                当月黒字転換日: {firstTurnPositiveDay}日
              </span>
            )}
          </div>
        </div>
            ...
            <thead>
              <tr className="bg-slate-100/90 text-slate-700 border-b border-slate-200 text-[11px] font-bold">
                ...
                <th className="py-2.5 px-3 text-right whitespace-nowrap">
                  {perspective === 'hall' ? '当日粗利(G連動)' : '当日客収支'}
                </th>
                <th className="py-2.5 px-3 text-right whitespace-nowrap bg-indigo-50/50 min-w-[150px]">
                  1日平均比 乖離 (温度感バー)
                </th>
                <th className="py-2.5 px-3 text-right whitespace-nowrap bg-sky-50/60 min-w-[170px]">
                  積み上げ累積乖離 (ペース水準)
                </th>
                {tableMode === 'full' && (
                  <>
                    <th className="py-2.5 px-3 text-right whitespace-nowrap">月初累計粗利</th>
                    ...
                    <td className="py-2 px-3 text-right whitespace-nowrap bg-indigo-50/20">
                      <div className="space-y-1">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          >
                            {isDevPositive
                              ? perspective === 'hall' ? '黒字回収' : '客負け'
                              : perspective === 'hall' ? '出玉還元' : '客勝ち'}
                          </span>
                          <span
                            className={`font-black text-xs ${
                              isDevPositive ? 'text-emerald-700' : 'text-rose-600'
                            }`}
                        ...
                        <div className="w-full bg-slate-200/80 h-1.5 rounded-full overflow-hidden flex">
                          {isDevPositive ? (
                            <div
                              className="bg-emerald-500 h-full rounded-full ml-auto transition-all"
                              style={{ width: `${Math.max(devRatio, 4)}%` }}
                              title={`上振れ強度: ${devRatio}%`}
                            />
                          ) : (
                            <div
      ...
      {/* 4. Collapsible Macro Factors (特日・通常日・全期間基準) */}
      <div className="pt-2 border-t border-slate-200">
        <button
          type="button"
          onClick={() => setShowMacroCards(!showMacroCards)}
          className="text-xs text-indigo-700 hover:text-indigo-900 font-bold flex items-center gap-1 cursor-pointer transition-colors"
        >
          {showMacroCards ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
          {showMacroCards ? '特日・通常日・全期間基準マクロ要因カードを隠す' : '特日・通常日・全期間基準マクロ要因カードを展開する'}
        </button>

        {showMacroCards && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
            {/* Card 1: 1日平均ホール粗利 乖離分析 */}
            <div className="p-3.5 rounded-lg border border-indigo-200/80 bg-gradient-to-b from-indigo-50/40 to-white">
              ...
              <div className="space-y-1.5 text-xs text-slate-600">
                <div className="flex justify-between items-baseline">
                  <span className="text-slate-500">当月1日平均ホール粗利:</span>
                  <span className="font-extrabold text-slate-900 text-sm">
                    {formatYenExact(monthDailyAvg)} /日
                  </span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="text-slate-500">全期間全体1日平均基準:</span>
                ...
                <div className="pt-1.5 border-t border-indigo-100 flex justify-between items-baseline">
                  ...
                  <span
                    className={`font-black text-sm ${
                      diffFromOverall >= 0 ? 'text-indigo-700' : 'text-rose-600'
                    }`}
                  >
                    {diffFromOverall >= 0 ? '+' : ''}
            ...
            <div className="p-3.5 rounded-lg border border-rose-200/80 bg-gradient-to-b from-rose-50/40 to-white">
              ...
              <div className="space-y-1.5 text-xs text-slate-600">
                <div className="flex justify-between items-baseline">
                  <span className="text-slate-500">特日ホール粗利合計:</span>
                  <span className="font-bold text-slate-900">
                    {formatYenExact(eventProfit)}
                  </span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="text-slate-500">特日 1日平均粗利:</span>
                  <span className="font-extrabold text-slate-900 text-sm">
                ...
                <div className="pt-1.5 border-t border-rose-100 flex justify-between items-baseline">
                  ...
                  <span
                    className={`font-black text-sm ${
                      eventDiffFromMonth >= 0 ? 'text-indigo-700' : 'text-rose-600'
                    }`}
                  >
            ...
            <div className="p-3.5 rounded-lg border border-slate-200 bg-gradient-to-b from-slate-50/40 to-white">
              ...
              <div className="space-y-1.5 text-xs text-slate-600">
                <div className="flex justify-between items-baseline">
                  <span className="text-slate-500">通常日ホール粗利合計:</span>
                  <span className="font-bold text-slate-900">
                    {formatYenExact(normalProfit)}
                  </span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="text-slate-500">通常日 1日平均粗利:</span>
                  <span className="font-extrabold text-slate-900 text-sm">
                ...
                <div className="pt-1.5 border-t border-slate-200 flex justify-between items-baseline">
                  ...
                  <span
                    className={`font-black text-sm ${
                      normalDiffFromMonth >= 0 ? 'text-indigo-700' : 'text-rose-600'
                    }`}
                  >
      ...
      <div className="mx-4 sm:mx-5 my-3 p-3.5 bg-slate-50/90 rounded-lg border border-slate-200 text-xs">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-2 pb-2 border-b border-slate-200">
          ...
          <div className="text-slate-500 text-[11px]">
            全{periodBaseline.totalDays}営業日 / 累計粗利 {formatYen(periodBaseline.totalProfit)}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="bg-white p-2.5 rounded-md border border-indigo-100 shadow-2xs">
            <div className="text-[11px] text-slate-500 font-medium">全期間 1日平均ホール粗利（全体基準値）</div>
            ...
            <div className="text-[10px] text-slate-500 mt-0.5 font-medium">
              1台あたり日当: <span className="font-bold text-indigo-900">{formatYen(periodBaseline.overallPerMachineDaily)}/台・日</span>
            </div>
          </div>
