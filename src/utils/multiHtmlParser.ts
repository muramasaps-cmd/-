...
/**
 * Normalizes store name for grouping (trims and lowercases whitespace and fullwidth characters)
 */
...
export function parseMultipleSlorepoHtml(
  ...
  for (const [, group] of groupMap.entries()) {
    ...
    const normalizedGroupName = normalizeStoreNameKey(group.storeName);
    const existingMatch = existingStores.find(
      (s) => normalizeStoreNameKey(s.name) === normalizedGroupName
    );
